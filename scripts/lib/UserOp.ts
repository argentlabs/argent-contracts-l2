...
export async function fillAndSign(op: Partial<UserOperation>, signerOrSigners: Signer | Signer[], entryPoint?: EntryPoint): Promise<UserOperation> {
  let op1 = {...op}
  ...
  if (Array.isArray(signerOrSigners)) {
    const signatures = await Promise.all(signerOrSigners.map(async (signer) => await signer.signMessage(message)));
    op2.signature = ethers.utils.hexlify(ethers.utils.concat(signatures));
  } else {
    op2.signature = await signerOrSigners.signMessage(message);
  }
  return op2;
}
